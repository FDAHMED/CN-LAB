...
### IMPLEMENTATION OF ITERATIVE ECHO SERVER USING CONNECTION-LESS SERVICE(UDP) 
--- 
...
- Server blocks for receiving the message from the client by using recvfrom system 
call. 
- Sends the message by echoing to client using sendto system call. 

#### Client side : 
- First creates the socket. 
- Initializes the server address to sockaddress_in structure to which connect request 
has to send. 
- Sends the message to server using sendto system call byspecifying server address 
along with the message. 
- Then waits for the echo message from the server with recvfrom system call. 
