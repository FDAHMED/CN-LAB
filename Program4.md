## Program 4 
---
### IMPLEMENTATION OF CONCURRENT ECHO SERVER USING CONNECTION-ORIENTED(TCP) 
---
#### Description: 
...
#### Server side : 
---
- First creats the socket using socket system call. 
- Binds the name (i.e. IP address, port number and family) of the server to the socket. 
- Makes the socket to be ready in passive state by creating queues for storing the 
 client requests using listen system call. In this server is listening for the clients. 
- Once the connect request comes from the client, it stores in the queue and calls 
...
- Creates the child process using fork system call. 
- Receives and sends the messages from/to the client in the child process. 
- Parent process waits for the next client. 
- Close the system call after completing the communication. 
 ---
#### Client side : 
---
- First creates the socket. 
- Initializes the server address to sockaddress_in structure to which connect request 
has to send. 
- Makes a connection request to the server using connection system call. If the 
...
- If connection is established,sends and receives the messages using send and recv system calls. 
